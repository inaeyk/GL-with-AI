...
No final overall score is assigned before hidden-aware cleanup/constraints
and actual evolution runs. The custom outer WKB/boundary-bearing
...
|---|---|---|---|---|---|---|---|---|---|---|
| Custom/stock state layout | Real custom `UserVariables.hpp`, frozen-state header, and namespaced stock `CCZ4UserVariables.hpp` | Direct inspection of both actual enums | exact custom 18-slot layout; eleven documented shared correspondences; two hidden representatives, zero visible-y fields, and five gauge slots | N/A | N/A | N/A | swapped visible slots, hidden-to-visible-y, duplicate, and omission mutations rejected | fixture reconciled when E1 retired the historical 27-slot scaffold | retain focused regression | batch-1 results |
...
| Modified-cartoon target input seam | Reduced `(2+2)` value/jet expansion to `(x,z,w1,w2)` | High at the mapping boundary; full RHS remains GRChombo-owned | complete pointwise target-`d=4` expansion with multiplicity two and one stored representative | complete; active multiplicity, `1/x^2`, cartoon, encoded-Z/Gamma, and representative-evolution mutations rejected | live real-derivative fixture passes | E1 integration only; no evolution | Old Christoffel-order and Hessian-index oracle defects restored as active failing regressions | direct nonlinear comparison exposed both oracle defects | retain validated input seam | repaired target pointwise and E1 live fixtures |
...
| Direct full target-`d=4` 13-row RHS | Reduced expansion plus direct locked GRChombo `CCZ4RHS::rhs_equation`; independent custom oracle | High: oracle does not call production and production does not call the complete oracle assembler | complete direct target evaluation; no independently coded hidden-family RHS | complete direct nonlinear equivalence for all 13 rows; maximum absolute `1.2216790079566664e-15`, normalized `0.0023890868613152456` | nine live cells and all 18 physical/gauge slots match exactly | E1 integration only; no evolution | target-input mutations plus both restored oracle-defect mutations fail | corrected two-pass Christoffel and vector-Hessian ordering required | retain as primary pointwise gate | repaired target pointwise and E1 live fixtures |
| Hidden-increment decomposition | Two direct locked GRChombo evaluations | Decomposition evidence, not an independent hidden implementation | `target_full_grchombo - target_shared_hidden_suppressed` | complete as observable decomposition evidence; exact GP cancellation retained | N/A | not wired live | representative-output duplication rejected as reporting-layer mutation | terminology/ownership repair required | retain subtraction report; independently coded hidden-family RHS is not implemented and is not required by this architecture | repaired target pointwise fixture |
| Target directional JVP diagnostic | Central differences of direct GRChombo and independent oracle finite states | High, but quotient is roundoff/cancellation sensitive | no convergence claim | secondary diagnostic: roundoff/cancellation dominated across the tested epsilon range, with approximately `1/epsilon` amplification | N/A | N/A | all epsilons reported without a selected-epsilon or tolerance gate | former stable-window claim removed | retain diagnostic only | repaired target pointwise fixture |
...
| GRChombo production infrastructure | Direct reuse | N/A | authority; full target-`d=4` CCZ4/geometry source executes directly | pointwise direct target evaluation complete | E1 real storage, BoxLoop, derivatives, and periodic exchange pass | no sustained evolution | wrong-dimension and target-input controls | no production-source intervention | direct reuse | repaired target pointwise fixture, E1 fixture, and batches 1-4 |
...
| Visible algebraic cleanup | Independent same-dimension matrix oracle versus actual `TraceARemoval` and `PositiveChiAndAlpha` | High | visible `d=3` trace denominator and positivity convention match; no stock determinant-normalization compute class found | component max `2.776e-17`, normalized below Level-2 limit; post-trace `5.551e-17` | direct cleanup/idempotence checks pass | N/A | wrong dimension, omission, hidden multiplicity, omitted positivity, and source-only determinant controls detected | no batch-3 human intervention | retained as stock-overlap evidence | batch-3 results |
| Target hidden-aware cleanup, constraints, and fixed source | Reduced target seam; direct locked `CCZ4Geometry::compute_ricci`; source-convention formula layer from `Constraints.impl.hpp`; direct `MovingPunctureGauge`; independent long-double custom constraint oracle | High at the pointwise boundary; complete stock `Constraints` is source/convention-only because its pointwise method is protected and its derivative storage is `CH_SPACEDIM`-owned | determinant and weighted trace use two hidden copies; `H=R+3K^2/4-A_IJ A^IJ`; exactly `H,Mx,Mz`; fixed source is lapse-only | cleanup determinant `1`, weighted trace `2.776e-17`; constraint max absolute `6.403298030699e-16`, normalized `1.276405780850e-3`; genuine `P_+`/`P_-` leakage and commutator are zero; GP sourced lapse roundoff zero | live cleanup/diagnostics/source fixture matches exactly and state mutation count is zero | E1 integration only; no evolution | old reconstructed-`K_IJ`, wrong `3/4`, hidden/off-diagonal/Ricci omissions, independent `Mx/Mz`, hidden derivative, `K` gradient, off-diagonal momentum, real reduction leakage, cleanup, and source mutations fail | no final score assigned | proceed to bounded unperturbed evolution | repaired cleanup/constraint/source and E1 fixtures |
...
| Exact matches | Discrete conventions, integers, signs, rational coefficients, or slot maps match exactly | Level-1 result record | repaired custom 18-slot permutation, eleven shared correspondences, and explicit compile-time dimension identities |
...
|---|---|---|---|---|---|
| 2026-07-23 / 2026-07-26 | Batch-1 state mapping | Require the executable fixture to validate the real application enum, then reconcile it when E1 retired the historical 27-slot scaffold | The original gate did not prove numeric custom slots; after E1, retaining the 27-slot assertion would contradict production | Exact-map evidence now covers the 18-slot, eleven-shared, zero-visible-y, five-gauge contract | batch-1 results |
...
The target pointwise checkpoint adds direct locked-source evidence for all 13
physical rows, direct nonlinear independent-oracle comparison, genuine
Fourier-sector checks, GP hidden-suppressed/full decomposition, and active
target-input mutations. Both formerly corrected oracle defects now have
active failing regression paths. The directional JVP is only a
roundoff/cancellation-dominated secondary diagnostic and supplies no
convergence or completion evidence. The hidden increment is defined by
subtraction; no independently implemented hidden-family RHS is claimed or
required by the current architecture. E1 adds live `BoxLoop`
RHS/cleanup/constraint/source and periodic-ghost evidence, but no final agent
score: unperturbed evolution, accepted radial-boundary behavior, and later
...
## Stage 4AO-D-E1 evidence note

The isolated live application now supplies real Chombo storage/traversal,
locked fourth-order derivatives, direct target-`d=4` physical RHS ownership,
five gauge rows, one lapse-only fixed source, post-update hidden-aware
cleanup, observational `H,Mx,Mz`, and periodic direction-1 ghost exchange.
All 18 initialized and RHS slots match their accepted seams in the focused
fixture, and periodic manufactured derivatives show fourth-order convergence.

...
| GRChombo production infrastructure | Direct reuse | N/A | authority; full target-`d=4` pointwise CCZ4/geometry source executes directly | pointwise direct target evaluation complete; live Chombo BoxLoop RHS remains unwired | TBD | TBD | wrong-dimension and target-input controls | no production-source intervention | direct reuse | repaired target pointwise fixture plus batches 1-4 |
...
required by the current architecture. This does not add a final agent score:
live BoxLoop cleanup/constraint/source wiring and evolution evidence remain
missing; the pointwise cleanup/constraint/source evidence is complete.

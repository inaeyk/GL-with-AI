...
until a checked local off-diagonal `h_xz / x` ingredient exists: the gradient
expansion contains both the Stage 4Q/4R/4U-protected `(hxx - hww) / x^2` path
and an unguarded `h^xz h_xz / x^2` contribution. Stage 4W derives
`h_xz = O(x)` from reflection parity about the cartoon axis and records that
the curvature/lapse RHS sign must be pinned against the published CCZ4/BSSN
formulation and the Stage 3A `K_IJ` convention caveat. The next narrow coding
stage should therefore be Stage 4X, a checked local `h_xz / x` ingredient, not
an `R_ww[gamma]` implementation. A stronger grid-level or two-sided parity
validation of `h_xz = O(x)` remains future work.

## Stage 4X Checked Hxz-Over-X Ingredient

Stage 4X adds the local checked off-diagonal `h_xz / x` ingredient needed by
future hidden-sphere source formulas:

- Checked quotient:
  `code/BlackStringToy/CartoonCheckedHxzOverX.hpp`.
- Fixture:
  `code/BlackStringToy/tests/Stage4XCheckedHxzOverXIngredientTest.cpp`.

The helper forms the away-axis quotient `h_xz / x` through the existing
`CartoonAxisPolicy` path and packages it in a non-aggregate checked type.
Finite nonzero quotients are allowed, because regularity permits
`h_xz = x q(x^2,z)` with finite nonzero `q`. Invalid axis values and
nonfinite inputs reject.

Stage 4X is still only a local checked quotient/token. It verifies that
`h_xz / x` is finite at the supplied away-axis point. It does not prove global
grid parity, prove the analytic statement `h_xz = O(x)`, implement the
`h^xz h_xz / x^2` source expression, implement `tilde{R}_ww[h]`,
`R^chi_ww`, or physical `R_ww[gamma]`, and it does not add full Ricci, CCZ4
RHS terms, finite-axis regularization, grid reads, or evolution wiring. A true
...
After Checkpoint B, its reduced determinant policy matches Stage 4Z, Stage
4AB, and Stage 4AC: finite positive `D = h_xx h_zz - h_xz^2` is required, and
finite negative determinants are rejected.

## Stage 4Z Conformal Rww First-Derivative Gradient Block

Stage 4Z adds the next guarded formula sub-block from the conformal hidden
Ricci target:

- Checked quotient:
  `code/BlackStringToy/CartoonCheckedDxhwwOverX.hpp`.
- Formula block:
  `code/BlackStringToy/CartoonConformalRwwGradientBlock.hpp`.
- Fixture:
  `code/BlackStringToy/tests/Stage4ZConformalRwwGradientBlockTest.cpp`.
...
with `A = h_xx`, `B = h_xz`, `C = h_zz`, `W = h_ww`,
`D = A C - B^2`, `q_xz` consumed from the Stage 4X checked `h_xz / x`
package, and `p_W = W_x / x` consumed from the new Stage 4Z checked package.
The source-facing API consumes a single non-forgeable
`ConformalRwwGradientBlockInputs` package minted from one local metric point:
`x`, `h_xx`, `h_xz`, `h_zz`, `h_ww`, `d_x h_ww`, and `d_z h_ww`.

Stage 4Z rejects invalid axis values, nonfinite local values, nonpositive
`h_ww`, and zero/nonfinite/nonpositive reduced determinant. It is not the
Stage 4Y singular block, not the Hessian block, not full `tilde{R}_ww[h]`,
not `R^chi_ww`, not physical `R_ww[gamma]`, not full Ricci, not CCZ4 RHS, and
not evolution wiring. It is away-axis only and does not prove global parity or
finite-axis regularity.

## Stage 4AA Conformal Rww Hessian Derivation Lock

Stage 4AA is the documentation-only Hessian derivation lock for the conformal
hidden Ricci target. It records
...
along with the `rho` derivative formulas, reduced-base Christoffels, and
Hessian oracles. The locked numeric oracles are flat `0`, constant cone `0`,
and nonconstant `W = (1 + x)^2` at `x = 1` giving `G^Hess_ww = -4`; this
combines with the existing Stage 4Y/4Z values to give `tilde R_ww = -12` for
that conformal fixture. Claude Audit A verifies the fully distinct
nonsymmetric sample with all derivative slots nonzero:
`G^Hess_ww = -8558 / 2883`, approximately `-2.9684356573014221`. For the same
sample, `G^sing_ww + G^grad_ww + G^Hess_ww = -3576 / 961`, approximately
`-3.7211238293`, matching the independent Stage 4G conformal Ricci engine to
machine precision with residual about `4.44e-16`.

The verified nonsymmetric oracle is required for Stage 4AB because it
exercises off-diagonal Christoffels, `rho_xz`, `W_z`-dependent terms, and the
`(-2B/D)` contraction. The flat, constant-cone, and `W=(1+x)^2` oracles are
not sufficient by themselves. Stage 4AA adds no code, no tests, no RHS wiring,
and no evolution path. Stage 4AB may now implement the Hessian block, but only
with this verified nonsymmetric oracle included in the test.

## Stage 4AB Conformal Rww Hessian Block

Stage 4AB adds the local away-axis conformal Hessian sub-block:

- Formula block:
  `code/BlackStringToy/CartoonConformalRwwHessianBlock.hpp`.
- Fixture:
...
with `H_ab = rho_ab - Gamma^x_ab rho_x - Gamma^z_ab rho_z` and the
Stage 4AA `rho` derivative and Christoffel formulas. The source-facing API
consumes a single non-forgeable `ConformalRwwHessianBlockInputs` package
minted from one local point: `x`, `A`, `B`, `C`, `W`,
`A_x`, `A_z`, `B_x`, `B_z`, `C_x`, `C_z`, `W_x`, `W_z`, `W_xx`, `W_xz`,
and `W_zz`.

Stage 4AB rejects invalid axis values, nonfinite local values, nonpositive
`W`, and zero/nonfinite/nonpositive reduced determinant. It does not accept
loose raw determinant values or precomputed Christoffel inputs. It is not the
Stage 4Y singular block, not the Stage 4Z gradient block, not full
`tilde{R}_ww[h]`, not `R^chi_ww`, not physical `R_ww[gamma]`, not full Ricci,
not CCZ4 RHS, and not evolution wiring. Stage 4AC remains the owner of full
conformal `tilde{R}_ww[h]` assembly.

## Stage 4AC Conformal Rww Assembly

Stage 4AC assembles the local away-axis conformal hidden Ricci target from the
...
The source-facing API consumes a single non-forgeable `ConformalRwwInputs`
package minted from one local point. Its factory internally mints the
Stage 4Y singular-block input, Stage 4Z gradient-block input, and Stage 4AB
Hessian-block input from the same `x`, reduced metric values, reduced-base
first derivatives, and `W` first/second derivatives. The assembler does not
accept loose raw block values, loose raw quotient values, or a precomputed
determinant.

The fixture checks flat `0`, constant-cone `-3/4`, nonconstant
`W=(1+x)^2` giving `-12`, and the Claude-verified nonsymmetric value
`-3576/961`. It also compares the nonconstant and nonsymmetric `chi=1` cases
against the independent Stage 4G metric-derivative Ricci helper. Stage 4AC is
still conformal `tilde R_ww[h]` only. It does not implement `R^chi_ww`,
physical `R_ww[gamma]`, full Ricci, CCZ4 RHS, grid reads, or evolution
wiring. Checkpoint B / Claude Audit B applied one pre-4AD cleanup: it unifies the
reduced determinant policy so the singular, gradient, Hessian, and conformal
assembly paths all reject `D <= 0`.
...
Stage 4AD-4AR gates: `R^chi_ww`, the
split-vs-direct physical Ricci identity test, true `h_xz` parity validation,
physical `R_ww[gamma]` assembly, hidden lapse Hessian,
...
| Stage 4Y conformal Rww singular-gradient sub-block | `CartoonConformalRwwSingularBlock.hpp`; `Stage4YConformalRwwSingularBlockTest.cpp` | Compute the checked singular/regularity-sensitive gradient sub-block of the conformal hidden Ricci target | Single-source `ConformalRwwSingularBlockInputs` minted from one local `x`, `h_xx`, `h_xz`, `h_zz`, `h_ww` point, carrying Stage 4U checked `Delta_xw`, Stage 4X checked `q_xz`, and determinant data | Named `G_sing = (h_zz / D) Delta_xw - q_xz^2 / D`; no full Ricci output | Stage 4X | flat, constant-cone, distinct-value oracle, non-aggregate single-source input package, checked-input type boundary, invalid ingredient rejection, zero/negative/nonfinite determinant rejection, no-full-Ricci/no-evolution guards | High |
| Stage 4Z conformal Rww first-derivative gradient sub-block | `CartoonCheckedDxhwwOverX.hpp`; `CartoonConformalRwwGradientBlock.hpp`; `Stage4ZConformalRwwGradientBlockTest.cpp` | Compute the checked first-derivative gradient sub-block of the conformal hidden Ricci target | Single-source `ConformalRwwGradientBlockInputs` minted from one local point, carrying Stage 4X checked `q_xz`, Stage 4Z checked `p_W = W_x / x`, raw `W_z`, and determinant data | Named `G_grad = -(C/D) p_W + (q_xz W_z)/D - (C W_x^2 - 2 B W_x W_z + A W_z^2)/(4 W D)`; no full Ricci output | Stage 4Y | flat, constant-cone, nonconstant-hidden-metric, distinct-value oracle, non-aggregate checked/input packages, invalid axis/`W`/determinant/derivative rejection, no-full-Ricci/no-evolution guards | High |
| Stage 4AA Hessian block derivation lock | `docs/implementation/stage4_hidden_sphere_Rww_plan.md`; `docs/derivations/stage4W_hidden_sphere_CCZ4_Rww_derivation.md` | Lock the conformal Hessian sub-block before code | Reviewed Stage 4Y/4Z context and reduced metric derivative notation | Documentation-only `G^Hess_ww` formula, `rho` derivatives, Christoffels, flat/cone/nonconstant oracles, and Claude Audit A's verified nonsymmetric oracle; no code | Stage 4Z | derivation formulas and oracles; nonsymmetric sample requires `G^Hess_ww=-8558/2883` and full conformal sum `-3576/961` in Stage 4AB testing | High |
| Stage 4AB conformal Rww Hessian sub-block | `CartoonConformalRwwHessianBlock.hpp`; `Stage4ABConformalRwwHessianBlockTest.cpp` | Compute the checked Hessian sub-block of the conformal hidden Ricci target | Single-source `ConformalRwwHessianBlockInputs` minted from one local point, carrying reduced metric values, base first derivatives, `W` first/second derivatives, and determinant data | Named `G_Hess = -(sqrt(W)/x)[(C/D)H_xx - (2B/D)H_xz + (A/D)H_zz]`; no full Ricci output | Stage 4AA | flat, constant-cone, nonconstant-`W`, verified nonsymmetric oracle, test-only conformal sum, non-aggregate input package, invalid axis/`W`/determinant/derivative rejection, no-full-Ricci/no-evolution guards | High |
| Stage 4AC conformal Rww assembly | `CartoonConformalRww.hpp`; `Stage4ACConformalRwwAssemblyTest.cpp` | Assemble the reviewed conformal hidden Ricci sub-blocks without physical Ricci claims | Single-source `ConformalRwwInputs` minted from one local point, internally carrying Stage 4Y, 4Z, and 4AB inputs | Named `tilde R_ww[h] = G_sing + G_grad + G_Hess`; no `R^chi_ww`, physical Ricci, RHS, or evolution output | Stage 4AB | flat, constant-cone, nonconstant-`W`, verified nonsymmetric full conformal oracle, non-aggregate input package, rejection through underlying factories, Stage 4G direct comparison for `chi=1` cases | High |
| Stage 4AD-4AR hidden-sphere Ricci/RHS roadmap | `docs/implementation/stage4_hidden_sphere_Rww_plan.md` | Own the remaining hard gates before hidden-sphere Ricci reaches RHS/evolution | Stage 4AC conformal assembly plus future `R^chi_ww`, parity, identity, and `hat_Gamma^x` gates | Roadmap beyond conformal assembly; no physical Ricci/RHS/evolution code | Stage 4AC and Checkpoint B | concrete future ownership for all high-risk hidden-sphere Ricci and CCZ4 gates | High |
...
| Conformal Rww first-derivative gradient sub-block | Stage 4Z fixture proving checked `p_W = W_x / x`, single-source metric/derivative inputs, the flat/constant-cone/nonconstant-`W`/distinct-value oracles, rejection cases, and no full Ricci/RHS/evolution |
| Hessian conformal Rww derivation lock | Stage 4AA documentation gate locking `G^Hess_ww`, `rho` derivatives, reduced Christoffels, flat/cone/nonconstant oracles, the combined `tilde R_ww=-12` check, and the verified nonsymmetric `G^Hess_ww=-8558/2883` / full sum `-3576/961` oracle required for Stage 4AB |
...
| Hidden `A_ww` curvature/lapse geometric core | Stage 4AK fixture proving same-point Stage 4AH plus Stage 4AJ composition, `-D_wD_w alpha + alpha R_ww[gamma]`, flat/cone/varying-chi/nonsymmetric oracles, rejection cases, and no Z4/trace-free/chi-prefactor/RHS/evolution claim |
| Trace-free curvature/lapse block | Stage 4AL fixture proving physical visible Hessians, Stage 4G/4I visible Ricci, Stage 4AH/4AK hidden core, full 4D trace-free projection, outer `chi`, flat/cone/varying nonsymmetric oracles, and no Z4/nonlinear/RHS/evolution claim |
| `hat_Gamma^x` derivation lock | Stage 4AM documentation gate proving the GRChombo `vars.Gamma` hatted convention, `Z_over_chi` translation, hidden contraction with multiplicity two, determinant-reduced cross-check caveat, required local oracles, and Gamma RHS term classification |
| `hat_Gamma^x` local contract | Stage 4AN fixture proving the local away-axis full contraction, checked `Delta_xw` and checked `B/x` consumption, hatted `Z_over_chi` convention, flat/cone/off-diagonal/distinct oracles, rejection cases, and no Gamma RHS/GL/evolution claim |
| Hidden-sphere Ricci roadmap | Stage 4AD-4AU plan proving the remaining gates have concrete ownership before physical `R_ww[gamma]`, `A_ww`, `hat_Gamma^x`, RHS, or evolution claims |
...
| Local `h_xz/x` token overclaimed as parity proof | Stage 4X and later axis-regularity claims | Stage 4X review, Stage 4AG, and Stage 4AP | Stage 4X is only a local checked quotient; Stage 4AG is synthetic paired-data validation only; actual grid/ghost-cell regularity validation waits for Stage 4AP |
...
| `hat_Gamma^x` hidden contraction under-tested by flat oracles | Connection/Gamma-driver sector and evolution claims | Stages 4AM-4AO and Checkpoint G | Stage 4AM locks the GRChombo hatted convention, hidden contraction, RHS map, and local oracles; Stage 4AN implements local contracts using checked singular ingredients; Stage 4AO must pass the hard GL growth/dispersion gate because flat checks alone are insufficient and Pau is not the convention authority |
...
Stage 4AG now provides only a standalone synthetic `h_xz` parity validator and
fixture. The validator uses paired positive/negative samples plus optional
axis data, checks odd-value and even-quotient residuals, and mints a
non-forgeable validated token. It neither clamps nor supplies finite-axis
values. It is not actual grid-data parity validation; Stage 4AP owns real
grid/ghost-cell regularity. The narrow Checkpoint D review now permits only
the local Stage 4AH assembly below.
...
Stage 4AK adds `CartoonAwwCurvatureLapseCore.hpp`. Its factory single-sources
the Stage 4AH physical `R_ww[gamma]` package and the Stage 4AJ physical
`D_wD_w alpha` package from one local metric/conformal-factor/lapse jet. It
computes `curvature_lapse_core = -D_wD_w alpha + alpha R_ww[gamma]` and
exposes the two signed pieces separately. This is not the full CCZ4 `A_ww`
block: Z4, trace-free projection, the outer `chi` factor, nonlinear A/K terms,
RHS, grid reads, finite-axis support, and evolution are still absent.

Stage 4AL adds `CartoonTraceFreeCurvatureLapseBlock.hpp`. Its single-source
factory builds the Stage 4G/4I physical Ricci path for `xx,xz,zz`, the Stage
4AH physical `R_ww`, and the Stage 4AK hidden core from one
metric/conformal-factor/lapse jet with visible lapse second derivatives. It
computes `C_IJ=-D_ID_J alpha+alpha R_IJ[gamma]`, projects with the full 4D
`1/4` trace-free denominator, and returns `chi C^TF_IJ`. The Stage 4G and
Stage 4AH physical `R_ww` values must agree before assembly. Checkpoint F /
Claude Audit F is recorded as complete after reviewing this local block.

Remaining Stage 4 source/evolution work is split into hard gates. Stage 4AM
now locks the `hat_Gamma^x` hidden contraction and maps the GRChombo
convention in `docs/derivations/stage4AM_hatGammaX_derivation.md`:
GRChombo evolves `vars.Gamma=chris.contracted+2 Z_over_chi`, so the project
shorthand `hat_Gamma^i=tilde_Gamma^i+2Z^i` must name the encoded
`Z_over_chi` convention explicitly. Stage 4AM also locks the full hidden
contraction, determinant-reduced cross-check caveat, required local oracles,
and Gamma RHS term map. Stage 4AN now implements the local away-axis
`hat_Gamma^x` contract in `CartoonHatGammaX.hpp`, consuming checked
...
sector, resolution, and measured growth variable. No live RHS/evolution
integration may proceed until 4AO passes and Checkpoint G reviews 4AM-4AO.
...
`h_xz=O(x)`, `h_xx-h_ww=O(x^2)`, `W_x=O(x)`, and `chi_x=O(x)`. Stage 4AQ owns
finite-axis source evaluation with explicit analytic limits/parity treatment:
no epsilon replacement, no silent clamping, and turduckening of the physical
singularity remains distinct from cartoon-axis regularity. Stage 4AR is
controlled local RHS integration; Checkpoint H covers 4AP-4AR before live
evolution. Stage 4AS is default-off live evolution wiring, Stage 4AT removes
or replaces the smoke-only hidden freeze in physics mode, and Stage 4AU is the
final Stage 4 exit review with Checkpoint I covering 4AS-4AU.

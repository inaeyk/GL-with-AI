...
- Began Stage 4B by adding a standalone variable-layout fixture for the public CCZ4 component order currently aliased by `BlackStringToy/UserVariables.hpp`. The check records visible helper-input bindings and current public slots before `K` and `Theta`, but does not edit enums, prove hidden `hww/Aww` placement, read grid variables, or wire Stage 4A helpers to evolution.

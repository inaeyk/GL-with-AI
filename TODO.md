...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. The encoded-Z helper and its isolated row-specific insertions now implement K `+=q`, Theta `+=q/2`, and A `+=qTF_IJ` without recomputing `q_IJ`, adding geometric Ricci, projecting twice, or doubling representative `ww`. The focused fixture verifies exact output ownership, weighted trace-free A output, scalar/even and xz/one-z parity, and coefficient/projection/Ricci-duplication mutations. Only the Gamma variable-completion flags remain true: K, Theta, every A row, the complete operator, and eigensolver stay incomplete. Next build the derivative adapter from `Z_i=0.5(H_i-g_i)` and assemble each complete non-Gamma row from common advection plus its already validated geometric, encoded-Z, algebraic, damping, and shift-stretching owners exactly once. Validate those assemblies with the deferred nonlinear selected-branch finite-difference oracle, background-zero checks, epsilon plateau, parity/trace-free checks, and omission/duplication mutations. Then remain full-operator JVP/parity, boundaries, MOTS, eigensolver/shift-invert, thresholds, convergence, the primary-source `k_c r0` map, and later 4AO-D. Frozen-gauge lapse-Hessian variation vanishes; locked `Lambda=0` leaves no cosmological K term.

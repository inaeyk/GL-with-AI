...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. The former K-output `A^2/K^2` block was removed as the wrong `USE_BSSN` branch. The 4AO-C wrapper contains the selected-CCZ4 K pieces `output[K] += 3 lambda input[K] - 3 lambda input[Theta]` and `output[K] += delta R`, alongside the existing boundary contract, GP-shift advection, tensor stretching, metric/chi coupling, A/Theta algebraic pieces, trace-free `delta A` projector, all four raw physical Ricci components and their trace/free assembly, Theta Ricci scalar insertion, and `A_IJ` trace-free Ricci curvature insertion. The main validation path deliberately locks `kappa1=0.1`, `kappa2=0`, `kappa3=1`, `covariantZ4=true` and implements the simple damping terms `output[Theta] += -0.25 input[Theta]`, `output[K] += -0.4 input[Theta]`; zero damping is reserved for a later diagnostic comparison. The hatted-Gamma design preflight locks both linearized rows, hidden multiplicities, Z reconstruction, parity, and common-advection ownership; the validation-only x/z contracted-connection helper implements `g_x`, full hidden-aware `g_z`, and `Z_i=0.5(H_i-g_i)` without writing RHS output. Separate non-advection Gamma blocks implement the Z/kappa/shift-gradient, K/Theta/chi-gradient, connection-A, vector-Hessian, and grad-div families. One assembler now consumes the existing common-advection output and adds each partial family exactly once. An independent nonlinear selected-CCZ4/modified-cartoon oracle validates both complete rows, exact GP background cancellation, directed-family and mixed perturbations, genuinely isolated vector-Hessian x and grad-div z cases, the `1e-5/1e-6` epsilon plateau, omission/duplication mutations, hidden multiplicity, parity, and output ownership. The generic Theta/Z constraint and hidden modified-cartoon inventory entries are implemented for both Gamma rows. The two Gamma RHS-completion flags are true; common advection is not duplicated, and neither direct `delta A_IJ` nor `d1.A` enters connection-A. The complete 13-variable operator remains false. Next implement remaining K/A/Theta and coupled CCZ4/Z4 pieces, Z/hat-Gamma-dependent Ricci, full-operator JVP/parity, boundary validation/convergence, linearized MOTS, eigensolver/shift-invert, threshold points, convergence battery, the primary-source `k_c r0` map, and later 4AO-D. Frozen-gauge lapse-Hessian variation vanishes; locked `Lambda=0` leaves no cosmological K term.

...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. The selected-CCZ4 frozen-gauge partial blocks and complete hatted-Gamma x/z rows are unchanged; only the Gamma variable-completion flags are true. The non-Gamma audit fixes `Q_IJ=2D_(I Z_J)` as the omitted completion of the existing geometric Ricci. A validation-only encoded-Z tensor helper now computes `q_xx=2dx z_x`, `q_xz=dx z_z+dz z_x`, `q_zz=2dz z_z`, the representative `q_ww=2z_x/x`, `q=q_xx+q_zz+2q_ww`, and `d=4` trace-free components from a supplied Z-derivative jet. Its analytic modified-cartoon oracle covers hidden multiplicity, projection, parity, zero, and ownership mutations. The helper neither expands `Z_i=0.5(H_i-g_i)` nor writes an RHS row. Next insertion targets are K `+=q`, Theta `+=q/2`, and A `+=qTF_IJ`; defer one nonlinear selected-branch finite-difference oracle until complete K/A/Theta row assembly to validate upper/lower-Z and conformal-factor mapping once. Then remain full-operator JVP/parity, boundaries, MOTS, eigensolver/shift-invert, threshold points, convergence, the primary-source `k_c r0` map, and later 4AO-D. Every non-Gamma completion flag and the full/eigensolver gates stay false. Frozen-gauge lapse-Hessian variation vanishes; locked `Lambda=0` leaves no cosmological K term.

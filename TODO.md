...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. All 13 frozen-gauge interior rows and the inner no-data pure-outflow endpoint are implemented and independently validated; all variable-completion and validation-only full-interior assembly/JVP/parity flags plus only the inner endpoint helper/validation flags are true. Next implement and independently validate the outer constraint-preserving decaying-subspace boundary, then run joint radial/boundary-location convergence before any eigensolver work. MOTS mapping, eigensolver/shift-invert, thresholds, the primary-source `k_c r0` map, and 4AO-D remain blocked. The aggregate radial-boundary, boundary-bearing complete-operator, and eigensolver gates remain false.

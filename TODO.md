...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. All 13 frozen-gauge interior rows are implemented and independently validated against analytic-jet selected-`USE_CCZ4` oracles; all variable-completion flags and the validation-only full-interior assembly/JVP/parity flags are true. The radial-boundary preflight now fixes the characteristic/outflow, asymptotic, stencil, ownership, and convergence contracts. Next implement and independently validate the inner pure-outflow endpoint closure, then the outer constraint-preserving decaying-subspace boundary, before any eigensolver work. MOTS mapping, eigensolver/shift-invert, thresholds, the primary-source `k_c r0` map, and 4AO-D remain blocked. The boundary-bearing complete-operator and eigensolver gates remain false.
...
- [ ] Stage 4AO-C: frozen-gauge spectral gate remains blocked/incomplete. The complete coupled frozen-gauge interior linearization and actual-operator JVP/parity checks pass, and `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md` now locks the radial-boundary design. Missing prerequisites are implementation and independent validation of those boundary operators, radial/boundary-location convergence, shift-invert or equivalent targeted extraction, the linearized MOTS/horizon observable map, primary-source `k_c r0` convention map, threshold zero crossing, and unstable/stable points.

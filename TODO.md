...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. The former K-output `A^2/K^2` block was removed as the wrong `USE_BSSN` branch. The 4AO-C wrapper contains the selected-CCZ4 K pieces `output[K] += 3 lambda input[K] - 3 lambda input[Theta]` and `output[K] += delta R`, alongside the existing boundary contract, GP-shift advection, tensor stretching, metric/chi coupling, A/Theta algebraic pieces, trace-free `delta A` projector, all four raw physical Ricci components and their trace/free assembly, Theta Ricci scalar insertion, and `A_IJ` trace-free Ricci curvature insertion. The main validation path now deliberately locks `kappa1=0.1`, `kappa2=0`, `kappa3=1`, `covariantZ4=true` and implements the simple damping terms `output[Theta] += -0.25 input[Theta]`, `output[K] += -0.4 input[Theta]`; zero damping is reserved for a later diagnostic comparison. Next resolve Z/hat-Gamma-dependent Ricci and complete both hatted-Gamma evolution rows, including Z/kappa and `kappa3` shift-gradient terms, then implement remaining coupled CCZ4/Z4 blocks, assembled-operator JVP/parity checks, boundary validation/convergence, linearized MOTS map, eigensolver/shift-invert, stable/unstable threshold points, the full convergence battery, the primary-source `k_c r0` map, and later the 4AO-D seeded-evolution bridge. Frozen-gauge lapse-Hessian variation vanishes; locked `Lambda=0` leaves no cosmological K term.

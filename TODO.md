...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. The former K-output `A^2/K^2` block was removed as the wrong `USE_BSSN` branch. The 4AO-C wrapper now includes only the selected-CCZ4 K pieces `output[K] += 3 lambda input[K] - 3 lambda input[Theta]` and `output[K] += delta R`, alongside the existing boundary contract, GP-shift advection, tensor stretching, metric/chi coupling, A/Theta algebraic pieces, trace-free `delta A` projector, all four raw physical Ricci components and their trace/free assembly, Theta Ricci scalar insertion, `A_IJ` trace-free Ricci curvature insertion, and the Z4 damping/kappa convention-lock derivation. Next resolve Z/hat-Gamma-dependent Ricci terms and the human `kappa1/kappa2` choice, then implement damping and hat-Gamma evolution, remaining operator blocks, actual-operator JVP/parity checks, linearized MOTS map, convention map, spectral targets, convergence tests, and later the 4AO-D seeded-evolution bridge. Frozen-gauge lapse-Hessian variation vanishes; locked `Lambda=0` leaves no cosmological K term.

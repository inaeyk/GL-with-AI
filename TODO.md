...
- [ ] Stage 4AO-C: frozen-gauge spectral gate remains blocked/incomplete. The complete coupled frozen-gauge interior linearization, actual-operator JVP/parity checks, inner pure-outflow endpoint validation, and outer `k>0` asymptotic-normalization design lock pass. Missing prerequisites are rank-nine outer-boundary implementation/validation, joint radial/boundary-location convergence, shift-invert or equivalent targeted extraction, the linearized MOTS/horizon observable map, primary-source `k_c r0` convention map, threshold zero crossing, and unstable/stable points.

...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. The full analytic modified-cartoon `Z_i=0.5(H_i-g_i)` derivative adapter and the one-time frozen-gauge K, Theta, and four A-row assemblers are implemented and independently validated against a nonlinear selected-`USE_CCZ4` oracle. K, Theta, all A rows, and both hatted-Gamma rows are now variable-complete; chi and the four metric rows remain incomplete. Next complete only those five rows, then perform the full 13-variable JVP/parity gate. Boundaries, MOTS, eigensolver/shift-invert, thresholds, convergence, the primary-source `k_c r0` map, and 4AO-D remain blocked. The complete-operator and eigensolver gates remain false.

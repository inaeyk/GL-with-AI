...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. The former K-output `A^2/K^2` block was removed as the wrong `USE_BSSN` branch. The 4AO-C wrapper contains the selected-CCZ4 K pieces `output[K] += 3 lambda input[K] - 3 lambda input[Theta]` and `output[K] += delta R`, alongside the existing boundary contract, GP-shift advection, tensor stretching, metric/chi coupling, A/Theta algebraic pieces, trace-free `delta A` projector, all four raw physical Ricci components and their trace/free assembly, Theta Ricci scalar insertion, and `A_IJ` trace-free Ricci curvature insertion. The main validation path deliberately locks `kappa1=0.1`, `kappa2=0`, `kappa3=1`, `covariantZ4=true` and implements the simple damping terms `output[Theta] += -0.25 input[Theta]`, `output[K] += -0.4 input[Theta]`; zero damping is reserved for a later diagnostic comparison. The hatted-Gamma design preflight locks both linearized rows, hidden multiplicities, Z reconstruction, parity, and common-advection ownership; the validation-only x/z contracted-connection helper implements `g_x`, full hidden-aware `g_z`, and `Z_i=0.5(H_i-g_i)` without writing RHS output. Separate non-advection Gamma blocks implement `(3 lambda/4)g_x - 0.2 Z_x + (lambda/2)H_x` and `(3 lambda/4)g_z - 0.2 Z_z`, the K/Theta/chi-gradient rows, the connection-A contractions `C_i = 2 A_{GP}^{IJ} delta Gamma^i_{IJ}` with both hidden `ww` copies, the vector-Hessian metric variation `(-3 lambda delta h_xx/(4x) + 3 lambda delta h_ww/x, 0)`, and the separately owned grad-div variation `(9 lambda delta h_xx/(8x), 9 lambda delta h_xz/(8x))`. The selected frozen-gauge Gamma term-family inventory is now mathematically closed, but final row assembly and assembled-row background/oracle/parity validation remain missing, so Gamma completion stays false. Common advection is not duplicated, and neither direct `delta A_IJ` nor `d1.A` enters the connection-A block. Next complete the Gamma row assembly/validation, Z/hat-Gamma-dependent Ricci, coupled CCZ4/Z4 blocks, assembled-operator JVP/parity checks, boundary validation/convergence, linearized MOTS map, eigensolver/shift-invert, stable/unstable threshold points, the full convergence battery, the primary-source `k_c r0` map, and later the 4AO-D seeded-evolution bridge. Frozen-gauge lapse-Hessian variation vanishes; locked `Lambda=0` leaves no cosmological K term.

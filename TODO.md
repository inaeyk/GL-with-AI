...
- [ ] Stage 4AO-C/4AO-D: complete the GL validation gate. All 13 frozen-gauge interior rows and the inner no-data pure-outflow endpoint are implemented and independently validated; the outer `k>0` normalization is now design-locked to four decaying amplitudes and nine explicit projector/Jordan/algebraic rows per parity sector. Next implement the narrow rank-nine transformed-amplitude/projector helper and its analytic-series/rank fixture, then compose the outer endpoint rows and run joint radial/boundary-location convergence before any eigensolver work. MOTS mapping, eigensolver/shift-invert, thresholds, the primary-source `k_c r0` map, and 4AO-D remain blocked. Outer implementation/validation, aggregate radial-boundary, boundary-bearing complete-operator, and eigensolver gates remain false.
...
- [ ] Stage 4AO-C: frozen-gauge spectral gate remains blocked/incomplete. The complete coupled frozen-gauge interior linearization, actual-operator JVP/parity checks, inner pure-outflow endpoint validation, outer `k>0` asymptotic-normalization design lock, and validation-only transformed-amplitude/rank-nine-projector helper pass. Missing prerequisites are actual rank-nine outer endpoint rows and their independent validation, joint radial/boundary-location convergence, shift-invert or equivalent targeted extraction, the linearized MOTS/horizon observable map, primary-source `k_c r0` convention map, threshold zero crossing, and unstable/stable points.

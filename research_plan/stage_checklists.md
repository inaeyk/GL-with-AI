...
- [ ] Stage 4AO-C: frozen-gauge spectral gate. Current status is blocked/incomplete in `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`: the intended operator is defined with frozen gauge perturbations, and the matrix-free GP-shift advection, tensor shift-stretching, algebraic metric/chi coupling, selected-CCZ4 K-output `+3 lambda delta K - 3 lambda delta Theta`, K-output physical `+delta R`, A-output non-curvature algebraic, Theta-output non-Ricci algebraic, Theta-output `-K_GP deltaTheta`, trace-free `delta A` projector contract, Ricci/curvature blocks, damping convention, and complete frozen-gauge hatted-Gamma x/z rows are implemented. The Gamma assembler consumes the existing common-advection output and adds the Z/kappa/shift-gradient, K/Theta/chi-gradient, connection-A, vector-Hessian, and grad-div blocks exactly once. Its independent nonlinear selected-CCZ4/modified-cartoon oracle validates exact GP background cancellation, directed-family and mixed cases, genuinely isolated vector-Hessian x and grad-div z cases, a stable `1e-5/1e-6` plateau, ownership mutations, hidden multiplicity, and parity. The generic Theta/Z and hidden modified-cartoon inventory entries are implemented for the completed Gamma rows. Only `variable_rhs_complete(hat_Gamma^x/z)` are true; the complete 13-variable operator remains false. Zero damping is reserved for a later diagnostic comparison. Remaining blockers: Z/hat-Gamma-dependent Ricci and remaining K/A/Theta/coupled CCZ4/Z4 RHS pieces; actual full-operator assembly and JVP/parity tests; boundary validation and convergence; linearized MOTS map; eigensolver/shift-invert; threshold/stable/unstable points; full convergence battery; and the primary-source `k_c r0` convention map. Frozen-gauge lapse-Hessian variation vanishes and cosmological terms remain absent under locked `Lambda=0`.

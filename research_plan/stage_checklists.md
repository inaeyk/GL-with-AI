...
- [ ] Stage 4AO-C: frozen-gauge spectral gate remains incomplete. All 13 interior rows are implemented and independently validated, all `variable_rhs_complete` flags are true, and the full validation-only interior assembly/JVP/parity flags pass. The radial-boundary continuum/discrete/validation contract is now designed, but no boundary operator or boundary test is implemented. Remaining blockers are boundary implementation and convergence, the linearized MOTS map, eigensolver/shift-invert, thresholds, and the primary-source `k_c r0` map. The boundary-bearing complete-operator and eigensolver gates remain false.

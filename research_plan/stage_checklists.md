...
- [ ] Stage 4AO-C: frozen-gauge spectral gate remains incomplete. The analytic encoded-Z derivative adapter and the complete K, Theta, four A, and two hatted-Gamma rows are implemented and independently validated. `variable_rhs_complete` is true exactly for those eight rows. Remaining blockers are the chi and four metric rows, complete 13-variable assembly/JVP/parity, boundaries, linearized MOTS, eigensolver/shift-invert, thresholds, convergence, and the primary-source `k_c r0` map. The full-operator and eigensolver gates remain false.

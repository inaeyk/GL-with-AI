...
The full frozen-gauge operator and eigensolver gate are still incomplete.
...
- [ ] Stage 4AO-C: frozen-gauge spectral gate. Current status is blocked/incomplete in `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`: the intended operator is defined with frozen gauge perturbations, and the matrix-free GP-shift advection, tensor shift-stretching, algebraic metric/chi coupling, selected-CCZ4 K-output `+3 lambda delta K - 3 lambda delta Theta`, K-output physical `+delta R`, A-output non-curvature algebraic, Theta-output non-Ricci algebraic, Theta-output `-K_GP deltaTheta`, trace-free `delta A` projector contract, Ricci/curvature design preflight, all four raw physical `delta R_IJ` validation blocks, raw Ricci trace/trace-free assembly, Theta Ricci scalar insertion, `A_IJ` trace-free Ricci curvature insertion, and Z4 damping/kappa convention-lock derivation pass are complete. Remaining blockers: Z/hat-Gamma-dependent Ricci terms, human `kappa1/kappa2` convention choice and Z4 damping implementation; hat-Gamma evolution; actual full-operator JVP/parity tests; boundary validation; linearized MOTS map; eigensolver/shift-invert; threshold/stable/unstable points; convergence battery; and the `k_c r0` convention map. Frozen-gauge lapse-Hessian variation vanishes and cosmological terms remain absent under locked `Lambda=0`.

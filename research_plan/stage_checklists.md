...
The boundary-bearing operator remains incomplete: radial conditions and
convergence, MOTS mapping, eigensolver/shift-invert, threshold results, and the
primary-source convention map are still blocked.
...
- [ ] Stage 4AO-C: frozen-gauge spectral gate remains incomplete. All 13 interior rows are implemented and independently validated, all `variable_rhs_complete` flags are true, and the full validation-only interior assembly/JVP/parity flags pass. The inner no-data pure-outflow endpoint operator and focused validation now pass, but the outer boundary and joint boundary-system convergence remain missing. Remaining blockers are outer implementation and full boundary convergence, the linearized MOTS map, eigensolver/shift-invert, thresholds, and the primary-source `k_c r0` map. The boundary-bearing complete-operator and eigensolver gates remain false.
...
- [x] Stage 4AO-C radial-boundary design preflight: trace the selected `USE_CCZ4` radial principal symbol and lock a blockwise physical/Z4 characteristic substitute. The light sectors have fixed-boundary speeds `1-sqrt(r0/x)` and `-1-sqrt(r0/x)`; shift-advected and frozen-longitudinal sectors have speed `-sqrt(r0/x)`. The frozen longitudinal sector contains a zero-normal-speed Jordan block, so no false complete characteristic basis is claimed. Every sector is outflow at `0<x_in<r0`; no inner continuum data are allowed, only second-order one-sided `D_x/D_xx/D_xz` closure and local determinant/weighted-trace cleanup. This preflight provisionally recorded `D_x+k+1/x` and a generic generalized layout; the dedicated outer-normalization lock below supersedes those two provisional details with the rank-nine WKB projector and sparse quadratic pencil. Homogeneous Dirichlet remains only a boundary-systematic alternative, and `k=0` remains a separate charge/gauge diagnostic. Boundary-row ownership, parity, hidden-ww normalization, mutation tests, and numerical convergence/location criteria are fixed in the derivation and fixture design. This item adds documentation only: every boundary, boundary-bearing complete-operator, MOTS, eigensolver, threshold, production, 4AO-D, and Checkpoint G gate remains false. The first implementation target was the independently testable inner pure-outflow endpoint derivative/packet-reflection block.
...
- [x] Stage 4AO-C outer `k>0` asymptotic-normalization lock: expand the actual complete stationary symbol rather than assuming componentwise `exp(-kx)/x`. In each full parity sector the finite asymptotic factor is `s^5(s^2-k^2)^4`: four admissible decaying light amplitudes and codimension nine. The two physical blocks have no `sqrt(x)` exponent; vector/scalar Z4 damping gives `exp[-0.1 sqrt(r0 x)]` and `exp[-0.5 sqrt(r0 x)]`, with `p_b^-=1+k r0/2+gamma_b^2 r0/(8k)`. Five additional homogeneous rows remove `J,F,G,C_h,C_A`; `F=G=0` removes both members of the frozen longitudinal Jordan chain without claiming a dynamical diagonalization. The decaying basis is normalized by its orthogonal projector through `x^-3/2`, giving nine explicit boundary rows and four retained outgoing PDE rows per sector. After harmless multiplication by `k`, the proposed explicit-row representation is a sparse quadratic polynomial pencil; rank-matched Dirichlet is only the systematic alternative. This item is documentation/design only. All outer implementation/validation, aggregate boundary, boundary-bearing complete-operator, eigensolver, MOTS, threshold, production, 4AO-D, and Checkpoint G gates remain false.
- [x] Stage 4AO-C outer `k>0` transformed-amplitude and rank-nine-projector helper: implement the validation-only `Y=(U,D_xU)` map to the four `W_b^{out/in}` pairs and `J,F,G,C_h,C_A`, with hidden multiplicity two and single representative `ww` ownership. Build the four decaying WKB columns with `gamma={0,0,0.1,0.5}`, corrected `p_b^-`, and stationary recursion through `x^-3/2`; form their scale/mixing-invariant orthogonal complement. The focused fixture proves rank nine/nullity four in both parity sectors, rejects growing/Jordan/charge/constraint directions, independently locks `F=G=0` without a tenth row, rejects normalization/multiplicity/rank mutations, and demonstrates continuum residual convergence faster than `O(x^-2)`. Only the two helper flags are true: no endpoint PDE row is replaced, and outer endpoint validation, aggregate boundary, boundary-bearing operator, eigensolver, MOTS, threshold, production, 4AO-D, and Checkpoint G remain false.

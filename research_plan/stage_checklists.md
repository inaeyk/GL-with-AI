...
- [ ] Stage 4AO-C: frozen-gauge spectral gate. Current status is blocked/incomplete in `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`. The existing partial operator and the complete frozen-gauge hatted-Gamma x/z rows are unchanged; only `variable_rhs_complete(hat_Gamma^x/z)` are true. The non-Gamma source audit fixes `Q_IJ=2D_(I Z_J)` and its row weights: K `+q`, Theta `+q/2`, and A `+[q_IJ]^TF`, with `q=q_xx+q_zz+2q_ww`. A validation-only encoded-Z tensor helper now supplies `q_IJ`, `q`, and `qTF_IJ` from an already formed Z-derivative jet, with an independent analytic modified-cartoon oracle. It does not include geometric Ricci, expand `Z_i=0.5(H_i-g_i)`, or write any RHS row. Remaining blockers include the derivative adapter, all three Z-completion row insertions, complete non-Gamma row assembly and its deferred nonlinear selected-branch finite-difference oracle, actual full-operator assembly/JVP/parity tests, boundaries, linearized MOTS, eigensolver/shift-invert, thresholds, convergence, and the primary-source `k_c r0` map. Every non-Gamma completion flag and the full/eigensolver gates remain false. Frozen-gauge lapse-Hessian variation vanishes and cosmological terms remain absent under locked `Lambda=0`.

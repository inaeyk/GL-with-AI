...
has started: the blocker/status note, reuse inventory, validation-only
operator wrapper, several partial algebraic/kinematic operator blocks,
trace-free `delta A` projector, Ricci design preflight, and raw lower/lower
physical `delta R_ww`, `delta R_xz`, `delta R_zz`, and `delta R_xx`
validation blocks plus raw Ricci trace/trace-free assembly are complete. The
Theta Ricci scalar insertion, trace-free A-curvature source insertion, and the
selected-CCZ4 K-row `K(K-2Theta)` / physical-`delta R` pieces are complete.
The inherited main-path kappa convention is locked and the simple K/Theta
damping insertions are complete. The full frozen-gauge operator and
eigensolver gate are still incomplete.
...
- [ ] Stage 4AO-C: frozen-gauge spectral gate. Current status is blocked/incomplete in `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`: the intended operator is defined with frozen gauge perturbations, and the matrix-free GP-shift advection, tensor shift-stretching, algebraic metric/chi coupling, selected-CCZ4 K-output `+3 lambda delta K - 3 lambda delta Theta`, K-output physical `+delta R`, A-output non-curvature algebraic, Theta-output non-Ricci algebraic, Theta-output `-K_GP deltaTheta`, trace-free `delta A` projector contract, Ricci/curvature design preflight, all four raw physical `delta R_IJ` validation blocks, raw Ricci trace/trace-free assembly, Theta Ricci scalar insertion, `A_IJ` trace-free Ricci curvature insertion, the inherited main-path convention `kappa1=0.1`, `kappa2=0`, `kappa3=1`, `covariantZ4=true`, the simple K/Theta damping insertions, the hatted-Gamma evolution design preflight, the validation-only x/z contracted-connection/Z reconstruction helper, the first non-advection hatted-Gamma Z/kappa plus `kappa3` shift-gradient insertion, the hatted-Gamma K/Theta/chi-gradient insertion, the hatted-Gamma connection-A metric insertion, and separate hatted-Gamma vector-Hessian and grad-div metric-variation insertions are complete. The surviving frozen-gauge Gamma mathematical term-family inventory is closed, but final Gamma row assembly and assembled-row background/oracle/parity validation remain missing, so neither Gamma row is complete. Zero damping is reserved for a later diagnostic comparison. Remaining blockers: Z/hat-Gamma-dependent Ricci terms; final assembly and validation of `hat_Gamma^x` and `hat_Gamma^z`; remaining coupled CCZ4/Z4 RHS pieces; actual assembled-operator JVP/parity tests; boundary validation and convergence; linearized MOTS map; eigensolver/shift-invert; threshold/stable/unstable points; full convergence battery; and the primary-source `k_c r0` convention map. Frozen-gauge lapse-Hessian variation vanishes and cosmological terms remain absent under locked `Lambda=0`.
...
- [x] Stage 4AO-C simple K/Theta damping insertion substep: with `d=4`, implement only `output[Theta] += -0.5*kappa1*(5+3*kappa2)*input[Theta] = -0.25 input[Theta]` and `output[K] += -4*kappa1*(1+kappa2)*input[Theta] = -0.4 input[Theta]`. No `deltaK` input or other output participates. Evidence: `code/BlackStringToy/Stage4AOFrozenGaugeOperator.hpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeCCZ4DampingInsertionTest.cpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeOperatorContractTest.cpp`; `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`; logs. The corrected K `+delta R + 3 lambda deltaK - 3 lambda deltaTheta` pieces remain unchanged, and this separate K/Theta damping block writes no hatted-Gamma output. Separate later partial Gamma blocks implement `(3 lambda/4)g_i - 0.2 Z_i` in both rows, `+(lambda/2)H_x` in the x row only, the K/Theta/chi-gradient family, connection-A, vector-Hessian, and grad-div metric variations. Complete hatted-Gamma evolution remains missing only at final row assembly and assembled-row validation.
- [x] Stage 4AO-C hatted-Gamma evolution design preflight: map the exact selected `USE_CCZ4` GRChombo `rhs.Gamma[i]` equation; derive both frozen-GP linearized rows with common advection ownership, `d=4` K/Theta/chi gradients, background shift-gradient and shift-Hessian terms, hidden multiplicity-two connection-A/vector-Hessian terms, `delta Z_over_chi^i=0.5(delta hat_Gamma^i-delta tilde_Gamma^i)`, locked `-0.2 delta Z_over_chi^i` damping, and scalar/even versus one-z parity. The selected GRChombo momentum-constraint form has no `d1.A`; no derivative-`delta A_IJ` term may be invented. Evidence: `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`; `docs/derivations/stage4AM_hatGammaX_derivation.md`; `external/GRChombo/Source/CCZ4/CCZ4RHS.impl.hpp`; logs. This remains the design authority; the contracted-connection helper plus the Z/kappa/shift-gradient, K/Theta/chi-gradient, connection-A, vector-Hessian, and grad-div blocks now close the surviving frozen-gauge mathematical family inventory, while final Gamma row assembly/validation, the complete operator, and eigensolver remain missing.
- [x] Stage 4AO-C contracted-connection/Z reconstruction helper: implement validation-only `g_x`, the full hidden-aware `g_z` companion, and `Z_i=0.5(H_i-g_i)` without assuming the determinant constraint or writing Gamma RHS output. Evidence: `code/BlackStringToy/Stage4AOFrozenGaugeContractedConnection.hpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeContractedConnectionTest.cpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeOperatorContractTest.cpp`; `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`; logs. Tests finite-difference the Stage 4AN nonlinear x contraction and an independent nonlinear z contraction, cover pure metric/Z and parity cases, and reject wrong hidden multiplicity, derivative signs, and unconditional determinant-reduced identities. The helper itself remains non-RHS; separate blocks now implement all surviving frozen-gauge Gamma mathematical families, while final row assembly/validation, operator completion, and eigensolver access remain missing.
- [x] Stage 4AO-C first hatted-Gamma Z/kappa and `kappa3` shift-gradient insertion: consume the validated `g_i,Z_i` helper and add only `output[hat_Gamma^x] += (3 lambda/4)g_x - 0.2 Z_x + (lambda/2)H_x` and `output[hat_Gamma^z] += (3 lambda/4)g_z - 0.2 Z_z`. The common GP-shift advection block remains the sole owner of `beta_GP^x partial_x H_i`; there is no `+(lambda/2)H_z`. Evidence: `code/BlackStringToy/Stage4AOFrozenGaugeOperator.hpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeHatGammaZ4KappaBlockTest.cpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeOperatorContractTest.cpp`; `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`; logs. The fixture independently derives coefficients from `K0=3 lambda/2`, `d=4`, `kappa1=0.1`, and `kappa3=1`, and rejects wrong damping sign, hidden multiplicity one, z-direction H coupling, duplicate advection, parity leakage, and false completion. All other surviving Gamma families are now implemented separately; final row assembly/validation, operator completion, and eigensolver access remain missing.
- [x] Stage 4AO-C hatted-Gamma K/Theta/chi-gradient insertion: add only `output[hat_Gamma^x] += 2 dx(deltaTheta) - (3/2)dx(deltaK) - 27 lambda delta h_xx/(8x) + (7 lambda/2)dx(delta chi)` and `output[hat_Gamma^z] += 2 dz(deltaTheta) - (3/2)dz(deltaK) - 27 lambda delta h_xz/(8x) + (3 lambda/2)dz(delta chi)`. The metric terms are the inverse-metric variation of `h^{ij} partial_j K_GP` with `partial_x K_GP=-9 lambda/(4x)`. Evidence: `code/BlackStringToy/Stage4AOFrozenGaugeOperator.hpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeHatGammaGradientBlockTest.cpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeOperatorContractTest.cpp`; `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`; logs. The focused oracle derives all coefficients independently from the selected d=4 equation and rejects wrong signs, omitted metric variation, swapped chi coefficients, parity leakage, non-Gamma writes, and duplicated earlier Gamma/advection terms. All other surviving Gamma families are now implemented separately; final row assembly/validation remains missing and all completion/eigensolver gates stay closed.
- [x] Stage 4AO-C hatted-Gamma connection-A insertion: add only `output[hat_Gamma^x] += C_x` and `output[hat_Gamma^z] += C_z`, where `C_i=2 A_{GP}^{IJ} delta Gamma^i_{IJ}` and the two hidden `ww` copies are explicit. The selected momentum-constraint form has no `d1.A`, while direct `delta A_IJ` vanishes because the background conformal Christoffels are zero. Evidence: `code/BlackStringToy/Stage4AOFrozenGaugeOperator.hpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeHatGammaConnectionABlockTest.cpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeOperatorContractTest.cpp`; `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`; logs. The independent oracle reconstructs each visible and hidden linearized conformal Christoffel and contracts it with `A_GP^xx=-7 lambda/8`, `A_GP^zz=-3 lambda/8`, `A_GP^ww=5 lambda/8`; it rejects hidden multiplicity one, wrong derivative and `1/x` signs, direct/differentiated `delta A`, parity leakage, non-Gamma writes, and duplicated prior Gamma/advection blocks. Vector-Hessian and grad-div are now implemented separately; final row assembly/validation remains missing and all completion/eigensolver gates stay closed.
- [x] Stage 4AO-C hatted-Gamma vector-Hessian and grad-div metric-variation insertion: keep the two selected-GRChombo families separate. Vector Hessian adds only `output[hat_Gamma^x] += -3 lambda delta h_xx/(4x) + 3 lambda delta h_ww/x` and zero to the z row, with both hidden directions explicit. Grad-div adds only `output[hat_Gamma^x] += 9 lambda delta h_xx/(8x)` and `output[hat_Gamma^z] += 9 lambda delta h_xz/(8x)`. Evidence: `code/BlackStringToy/Stage4AOFrozenGaugeOperator.hpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeHatGammaShiftMetricBlockTest.cpp`; `code/BlackStringToy/tests/Stage4AOCFrozenGaugeOperatorContractTest.cpp`; `docs/derivations/stage4AO_C_frozen_gauge_spectral_gate.md`; logs. Independent oracles derive `partial_xx beta_GP^x=3 lambda/(4x)`, each hidden `partial_ww beta_GP^x=-3 lambda/(2x)`, `partial_x div beta_GP=-9 lambda/(4x)`, and `(d-2)/d=1/2`; mutations reject hidden multiplicity one, omitted/wrong-sign `h_ww`, spurious z vector Hessian, wrong grad-div coefficient/sign, parity leakage, non-Gamma writes, and duplicated prior blocks. No mathematical Gamma RHS family remains unimplemented under the locked frozen-gauge specialization, but final row assembly and assembled-row background/oracle/parity validation remain missing; all completion and eigensolver gates stay closed.

...
- [x] Stage 4AO-C radial-boundary design preflight: trace the selected `USE_CCZ4` radial principal symbol and lock a blockwise physical/Z4 characteristic substitute. The light sectors have fixed-boundary speeds `1-sqrt(r0/x)` and `-1-sqrt(r0/x)`; shift-advected and frozen-longitudinal sectors have speed `-sqrt(r0/x)`. The frozen longitudinal sector contains a zero-normal-speed Jordan block, so no false complete characteristic basis is claimed. Every sector is outflow at `0<x_in<r0`; no inner continuum data are allowed, only second-order one-sided `D_x/D_xx/D_xz` closure and local determinant/weighted-trace cleanup. For `k>0`, the primary outer contract is the constraint-preserving asymptotic decaying subspace with leading Robin operator `D_x+k+1/x`; homogeneous Dirichlet is only a boundary-systematic alternative, and `k=0` remains a separate charge/gauge diagnostic. Boundary-row ownership, parity, hidden-ww normalization, sparse generalized-matrix layout, mutation tests, and numerical convergence/location criteria are fixed in the derivation and fixture design. This item adds documentation only: every boundary, boundary-bearing complete-operator, MOTS, eigensolver, threshold, production, 4AO-D, and Checkpoint G gate remains false. The first implementation target is the independently testable inner pure-outflow endpoint derivative/packet-reflection block.

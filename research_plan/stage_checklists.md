...
- [x] Stage 4AO-C outer `k>0` leading-asymptotic diagnostic: the leading stationary determinant factor `s^5(s^2-k^2)^4`, scalar profile powers `p_b^-=1+k r0/2+gamma_b^2 r0/(8k)`, and scalar logarithmic derivative including `S_b'/S_b` are documented. This does not derive the thirteen-component right-vector recursion or any dual left basis. Direct `W_b^in,J,F,G,C_h,C_A` conditions are diagnostic characteristic labels only; they are not the primary boundary condition. All outer and downstream gates remain false.
...
- [ ] Stage 4AO-C corrected `k>0` outer boundary: the rejected patch added valid mirrored second-order outer derivatives and useful `13+0/13+0/4+9` radial row-layout scaffolding, but used four one-hot transformed-amplitude columns scaled by scalar profiles. That normalization erased all scalar WKB data from the row space and supplied neither full 13-component jets nor a dual left basis. The nine direct characteristic selectors and four amplitude-coordinate PDE projections are diagnostic only. The interpolation-at-`k={1,2,3}` exact-pencil path and self-subtraction projector/Dirichlet claim were removed. Derive the full stationary-symbol recursion through `x^-3/2`, build and independently validate the true left nullspace and compatible retained-PDE dual, then reconsider the closed outer gates. The corrected boundary-bearing problem is nonlinear in `k` unless a later analytic proof establishes a polynomial or rational form.

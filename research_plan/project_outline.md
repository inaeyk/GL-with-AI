...
- Current status: design/tooling in progress with the first explicitly approved local C++ stages started. Stage 3A documents the target uniform-string initial data, GL perturbation, variable map, turduckening plan, and unresolved implementation decisions. Stage 3B documents the modified-cartoon hidden-direction source-term derivation and validation roadmap. Stage 3C adds a `docs/derivations/` SymPy scaffold for machine-checking basic Christoffels, GP extrinsic curvature, angular contractions, and hidden-direction trace multiplicity; the symbolic geometry check now passes and its output is recorded in the derivation notes. Stage 3D extends the symbolic scaffold to a diagonal nontrivial `gamma_ww = q(x,z)` metric and verifies hidden-sector Christoffel identities, angular Ricci multiplicity, and flat Ricci limits. Stage 3E adds a nonconstant `q(x,z)` warped-product Ricci regression that exercises hidden-sector derivative terms. Stage 3F translates the diagonal physical cartoon variables into conformal `chi`, `h_AB`, and `hww` bookkeeping and verifies derivative/tracelessness identities. Stage 3G extends the conformal-cartoon bookkeeping to an off-diagonal reduced metric with `h_xz` / `gamma_xz` and verifies determinant, inverse-metric, tracelessness, `K_xz`, denominator, diagonal-limit identities, and sheared-flat off-diagonal Ricci gates. Stage 3H decomposes the future CCZ4 RHS into separately checkable blocks, distinguishing exact anchors from external/reference-code and convergence-dependent validation routes before implementation. Stage 3I documents small-`x` parity, axis matching, the distinction between cartoon-axis regularity and the co-located physical singularity, removable-singularity limits, and the assembled `tilde_Gamma^x` / `hat_Gamma^x` connection guard while deferring finite-difference implementation choices. Stage 3J designs the unit-test fixture layer that maps the Stage 3C-3I symbolic gates to future C++ unit tests, RHS-block checks, reference-code comparisons, and convergence/evolution validation before source implementation. Stage 3K defines Stage 4A as the minimal first C++ implementation stage: repo-owned conformal-cartoon algebra helpers plus fixture tests, with GRChombo-facing convention authority documented and full RHS, gauge, damping, turduckening, AH, radiation, production runs, and external/reference comparison deferred. Stage 4A adds local-value-only conformal-cartoon algebra helpers and a standalone non-grid fixture. Stage 4B retains an automatic non-grid public CCZ4 baseline-layout fixture. Stage 4C adds real repo-owned `hww/Aww` variables plus header-level placement assertions and a non-grid placement fixture; the full scratch GRChombo scaffold compile succeeds with the 27-variable layout, while the inherited smoke run now fails until a later approved stage initializes or hands off the hidden variables. Stage 4D is the earliest planned grid-variable handoff. No grid-variable wiring, RHS, finite-difference, or evolution implementation has started.
